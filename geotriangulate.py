...
class Measurement(object):
    ...
    def __init__(self, lon: float, lat: float, alpha: float) -> None:
        self.sphcords: SphCoords = SphCoords(lon,lat)
        self.alpha: float = alpha
        self.__cartcord: CartCord = CartCord(0,0,0)
    ...
    @staticmethod
    def calculate_projection(cart: CartCord) -> SphCoords:
    ...
    @staticmethod
    def calculate_cart(sphcoords: SphCoords) -> CartCord:
    ...
    def calculate_self_cart(self) -> None:
        self.__cartcord = Measurement.calculate_cart(self.sphcords)

    def calculate_self_plane(self) -> None:
        if (self.__cartcord == CartCord(0,0,0)):
            raise RuntimeError("no cartesian")
        
        PoleCords: CartCord = Measurement.calculate_cart(NPoleCoords)
        ...
        unrot_vec = (x_proect - self.__cartcord.x, y_proect - self.__cartcord.y, z_proect - self.__cartcord.z)
        ...
        self.plane = Plane(
            self.__cartcord.y * meas_plane_vector[2] - self.__cartcord.z * meas_plane_vector[1],
            self.__cartcord.z * meas_plane_vector[0] - self.__cartcord.x * meas_plane_vector[2],
            self.__cartcord.x * meas_plane_vector[1] - self.__cartcord.y * meas_plane_vector[0],
            0
        )
    ...
    @staticmethod
    def find_inter(m1: Measurement, m2: Measurement) -> CartCord:
        ...
        dist_point1_to_m1 = sqrt((l1 * t1 - m1.__cartcord.x) ** 2 + (l2 * t1 - m1.__cartcord.y) ** 2 + (l3 * t1 - m1.__cartcord.z) ** 2)
        dist_point1_to_m2 = sqrt((l1 * t1 - m2.__cartcord.x) ** 2 + (l2 * t1 - m2.__cartcord.y) ** 2 + (l3 * t1 - m2.__cartcord.z) ** 2)
        dist_point2_to_m1 = sqrt((l1 * t2 - m1.__cartcord.x) ** 2 + (l2 * t2 - m1.__cartcord.y) ** 2 + (l3 * t2 - m1.__cartcord.z) ** 2)
        dist_point2_to_m2 = sqrt((l1 * t2 - m2.__cartcord.x) ** 2 + (l2 * t2 - m2.__cartcord.y) ** 2 + (l3 * t2 - m2.__cartcord.z) ** 2)

        if (dist_point1_to_m1 + dist_point1_to_m2 < dist_point2_to_m1 + dist_point2_to_m2):
            return CartCord(l1 * t1, l2 * t1, l3 * t1)
        else:
            return CartCord(l1 * t2, l2 * t2, l3 * t2)
...
class TargObj(object):
    ...
    _InterPoints: Dict[int, CartCord]
    ...
    def find_CurMedPos(self) -> None:
        ...
        self.CurMedPos = Measurement.calculate_projection(CartCord(x,y,z))
    ...
    def AddMeasurement(self, m: Measurement) -> Optional[int]:
        ...
        if (len(self._measurements) == 0):
            ...
            for cur_meas_id, cur_meas in self._measurements.items():
                ...
                new_inter_point: CartCord = Measurement.find_inter(m, cur_meas)
